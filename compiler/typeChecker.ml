...
(* Dummy - does nothing. *)
(*In future, we can add code to run through the Ast and update
the descr table with ocaml types in the tree *)

let gen dt current_descr tc loc decl = ([],[],current_descr)
