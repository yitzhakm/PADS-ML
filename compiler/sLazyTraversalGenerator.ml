...
let tool_pre = <:expr<$uid:N.gt_lazy_mod$.Rec_ver>>
...
let record_process_field = "process_field"
let record_process_last_field = "process_last_field"
let record_pre = <:expr<$lid:N.tool_rec$.$tool_pre$.record_t>>
...
let dt_process_empty = "process_empty_variant"
let dt_pre = <:expr<$lid:N.tool_rec$.$tool_pre$.datatype_t>>
...
let con_pre = <:expr<$lid:N.tool_rec$.$tool_pre$.constraint_t>>
...
let from_tid f_name loc tid d = 
  let specialize_fun = C.gen_TidTp loc "SLazyTraversalGenerator" tid N.specialize_lazy_fun d in
...
let tuple_to_fields tys =
  ...
  let convert (i,fds) ty = i+1,(PadscId.makeid (rid i),ty)::fds 
  in
...
and gen_traversal dt tc loc = function 
  ...
  | (SimpleAst.TyId _) as tp -> 
      let specialize_fun = gen_specialize dt tc loc tp in
	spec_and_project specialize_fun N.traversal_lazy_fun

  | (SimpleAst.TyApp _) as tp-> 
      let applied_specialize_fun = gen_specialize dt tc loc tp in
	spec_and_project applied_specialize_fun N.traversal_lazy_fun
  ...
  | SimpleAst.Datatype variants -> 
	...
	let vexpr = 
	  if C.contains_s_host_tp sub_tp then (
	    PadscError.report_warning loc 
	      "SLazyTraversalGenerator: host type used in branch; will not be traversable at run time.";
	    ...
	    <:expr<
		  ...
		  in
		  let s' = lazy ($gen_traversal dt tc loc sub_tp$ r pd s) in
	...
	(vpatt,None,vexpr)
      ...
      let mk_absorb_expr rep_con pd_con = 
		...
		let vexpr = <:expr<
	      $dt_pre$.$lid:dt_process_empty$ p_state $str:rep_con$
	      >>
		in
		  (vpatt,None,vexpr)
      in
      ...
      let all_cases = List.map gen_variant variants in
		<:expr<fun r (hdr,pd_body) state -> 
		  let p_state = $dt_pre$.$lid:dt_start$ state hdr in	  	    
			match (r,pd_body) with [$list:all_cases$]>> (* force? *)
  ...
  | SimpleAst.Constraint (tp, _, _) ->	
	  let traversal_fn = gen_traversal dt tc loc tp in
		<:expr<fun r (hdr,pd) state ->
		  ...
		  let s' = lazy ($traversal_fn$ r pd s) in
			$con_pre$.$lid:con_process$ p_state s'>> (* force *)
  | (SimpleAst.Table _) as tp ->
      let specialize_fun = gen_specialize dt tc loc tp in
	spec_and_project specialize_fun N.traversal_lazy_fun
...
and gen_specialize dt tc loc = function 
  ...
  | SimpleAst.TyId tid ->
	 ...
	 | Some d -> C.gen_TidTp loc "TraversalGenerator" tid N.specialize_lazy_fun d)
  ...
  | tp -> 
	...
	and $lid:N.traversal_lazy_fun$ = $traversal_fun$ in
	  {$lid:N.init_fun$ = $lid:N.init_fun$;
	   $lid:N.traversal_lazy_fun$ = $lid:N.traversal_lazy_fun$}>>
      in
...
let gen dt current_descr tc loc decl = 
  ...
  let mk_specialize_tool_ty arg_repty arg_pdbty = 
    <:ctyp<Generic_lazy_tool.Rec_ver.t 'in_state 'out_state 'rps 'dps 'cps 'lps
    ->  Type.SPIOTraversal.tool $arg_repty$ $arg_pdbty$ 'in_state 'out_state>>
  in

  let poly_t   = C.package_funty mk_specialize_tool_ty tp_params mk_specialize_tool_ty in
  let poly_fun = C.package_fun specialize_tool_body tp_params N.specialize_lazy_fun in
    
  let tp_pf_name = N.specialize_lazy_fun in
    (* create the final function *)
    ([], 
    ([<:sig_item<value $lid:tp_pf_name$ : $poly_t$>>],
    [<:str_item<open Generic_lazy_tool.Rec_ver>>;
     <:str_item<open Type.SPIOTraversal>>;
