...
(* Should the table be its own class rather than just parts of the
   module? *)
type table = metadata list
    
let new_table = []
  (* This string is the name of the pads type. Perhaps we should
     encode this with a special type other than "string"? *)
let add_descr dt info = info::dt
let add_table dt new_dt = new_dt @ dt
let lookup_descr dt name = 
  try 
    let d = List.find (fun d -> PadscId.eqid name d.name) dt in
      Some d
  with 
      Not_found -> None
