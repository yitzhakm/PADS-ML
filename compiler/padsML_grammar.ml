...
let pdecls = Grammar.Entry.create gram "pdecls"

let parse_pml input = Grammar.Entry.parse pdecls input

let pml_to_strings ast =
  List.map (function str_item -> Eprinter.apply pr_str_item Pprintf.empty_pc str_item) ast

let fprint_pml output ast =
  let s = pml_to_strings ast in
	List.iter (fun s ->
...
let print_pml ast =
  let ast = List.map (fun ast -> (ast, MLast.loc_of_str_item ast)) ast in
	(!print_implem) ast
...
EXTEND
  ...
  hlexpr :
    [[
	(* Need to limit to use of literals because full expressions
    ...
    | "<"; e = expr; ":"; t = ctyp; ">" -> HostLanguage.host_of_annotated_e e t
    |"pre"; id_e = LIDENT -> HostLanguage.host_of_regexp <:expr<$lid:id_e$>>
    |"pre"; str_e = STRING -> HostLanguage.host_of_regexp <:expr<$str:str_e$>>
(*    |"/"; str_e = STRING; "/" -> HostLanguage.host_of_regexp <:expr<$str:str_e$>> *)
    |"peol" -> HostLanguage.host_eol]]
  ;
  ...
  *)
  ...
  ptype : 
    [ "star" 
	[ tp = SELF;  "*"; tps = LIST1 (ptype LEVEL "ptype1") SEP "*"-> 
	    TupleTp (List.map tuple_flatten (tp::tps))]
    | "ws_star" 
	[ tp = SELF;  "*>"; tps = LIST1 (ptype LEVEL "ptype1") SEP "*>"-> 
	    (* prefix each tuple component with "pre ws" *)
	    let ws_re = TupleTp([Exp (HostLanguage.host_of_regexp <:expr<ws>>)]) in
	    let ws_tps = List.fold_right (fun x xs -> ws_re::x::xs) tps [] in
	    TupleTp (List.map tuple_flatten (tp::ws_tps))]
...
(* 	  tp = SELF; "Parray"; "["; args = LIST0 parray_args SEP ","; "]" -> ArrayTp (tp,args) | *)
	...
	| "("; arg_tp1 = SELF; ","; arg_tps = LIST1 SELF SEP ","; ")"; id = LIDENT -> 
	    TpAppTp (arg_tp1::arg_tps,make_type_id id)
...
(* 	| e = hlexpr -> SingletonTp e *)
	| e = hlexpr -> TupleTp([Exp e])
        ...
        | id = LIDENT -> TidTp (make_type_id id)
	| "[|"; tp = ctyp; "|]" -> HostTp(HL.tp_of_ctyp tp)
	] 
    ]
  ;
...
(*   ;	  *)
  ...
  ptype_vparam:
    [[ "("; id = LIDENT; ":"; t = ctyp; ")" -> (PadscId.makeid id, HostLanguage.tp_of_ctyp t)]]
  ; 
  
  ptype_tparam:
    [[ -> [] (*empty*) 
     | "("; ids = LIST1 LIDENT SEP ","; ")" -> List.map make_type_id ids
     | "'"; id = LIDENT -> [make_type_id id] ]]
    ;
  ...
  pcase:
    ...
    (* XXX: decide on "absorb" syntax. "of omit" is place holder. *)
      | p = patt; "->"; id = UIDENT; "of"; "omit"; o_tp = ptype -> 
	  AbsorbCase(hoc_p p, PadscId.makeid id, Type o_tp)
      | p = patt; "->"; id = UIDENT -> 
  ...
  pml_decl :
    [ [ "ptype"; tp_decls = LIST1 ptype_binding SEP "and" -> Ptype (loc, tp_decls)

    | "pextern"; ext_tp = pextern -> Pextern (loc, ext_tp) ]] 
  ;
  
  pstr_item :
	[[ decl = pml_decl -> decl 
	
	| decl = str_item -> Ocaml (HostLanguage.host_of_ocaml_si decl) ]]
  ;

  pdecls :
	[[ decls = LIST1 pstr_item -> decls ]]
