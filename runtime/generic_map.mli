...
*                                                                      *
***********************************************************************)
(** BTExtension defines the type used to encode the available base-type extensions. *)
module BTExtension : sig
  type t = IP | Other_ty
end
...
module type S = sig
   ...
   * in_data: the input at the moment of the error
   * string: An error msg *)
  exception Tool_error of in_data * string
  ...
  module type BaseType = sig
    ...
    val process  : in_data -> t Pads.result -> Pads.pd_header -> out_data
  end
  ...
  *)
  ...
  module Int    : BaseType with type t = int    
  module Float  : BaseType with type t = float  
  module Char   : BaseType with type t = char   
  module String : BaseType with type t = string 
  module Unit   : BaseType with type t = unit   
  (** Handles all extended base types that are not part of the core set. 
      Value provided by caller include a constructor indicating the type
      and a string encoding the value.
  *)
  module Extension : BaseType with type t = BTExtension.t * string
...
(** Functions for tuples and records. *)
  module Record : sig
	...
	(** Invocation: start num_fields is_tuple record_input record_pd_header *)
    val start : int -> boolean -> in_data -> Pads.pd_header -> partial_state

    (** 
      Retrieve the input for subcomponent "name" of a record.
    **)
    val project : in_data -> string -> in_data

    val process_field  : partial_state -> string -> out_data -> partial_state

    (**
       process the last field of the record and finish processing the entire record.
    **)
    val process_last_field  : partial_state -> string -> out_data -> out_data
  ...
  module Datatype : sig
    ...
    (**
       ...
       Returns: sum state
    **)
    val start : in_data -> Pads.pd_header -> partial_state

    (** 
      Retrieve the subcomponent input for tag "name" of a sum. 
      None is returned if there is no input corresponding to that name.
    **)
    val project : in_data -> string -> in_data option
    ...
    **)
    val process_variant : partial_state -> string -> out_data -> out_data

    (* For variants that have no contents. *)
    module Empty : sig
      (** Process an empty variant (that is, one that is just a tag). *)
      val process  : in_data -> out_data
    end
  ...
  module Constraint : sig
    ...
    **)
    val project : in_data -> in_data

    (**
      args: constrained-state underlying-state
      returns: new constrained-state
    **)
    val process : partial_state -> out_data -> out_data
...
(** Functions for lists. *)
  module List : sig
    ...
    val project_next : in_data -> in_data * in_data option

    val process_next  : partial_state -> out_data -> partial_state

    (** 
	Finish processing the list.
    **)
    val process_last  : partial_state -> out_data -> out_data
    ...
    val process_empty  : partial_state -> out_data
...
sig
  ...
  type ('state,'partial_state) datatype_t = {
      ...
      **)
      ...
      dt_init_empty   : unit -> 'state;
      
      (** Process an empty variant. *)
      dt_process_empty  : 'state -> 'state;
    }
  ...
  type ('state, 'record_p_state, 'datatype_p_state, 
	  ...
	  extension_t : (BTExtension.t * string, 'state) base_type;
