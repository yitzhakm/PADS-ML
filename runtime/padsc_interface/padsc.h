...
/* CAMLIDL related change:
   in reality, pflags_t is defined as 
   typedef Puint32 Pflags_t;
   However, we map Puint32 to a signed int64, which
   is totally unecessary here, as the flags is merely
   bit mask. So, we redefine as Pint32.
 */
...
void padsml_ml2c_ ## module ## _ ## type ## Opt(value input, type ## Opt * output);
...
value padsml_c2ml_ ## module ## _ ## type ## Opt( type ## Opt * input);
...
typedef RBuf_t *RBufPtr;
typedef Sfio_t *SfioPtr;
typedef SfioPtr SfioPtrOpt;
...
Perror_t PDCI_report_err(P_t *pads, int level, Ploc_t *loc,
			 PerrCode_t errCode, const char *whatfn, const char *msg, ...);

/*
 * Getter and setter functions for handle's discipline.
 */
