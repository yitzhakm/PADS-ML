...
module Make_plist (Specs: Plist_specs) =
  Make_plist.Make 
    (struct
      type ('a_rep, 'a_pdb) val_param_type = Specs.sep * Specs.term
    ...
    (struct
      type ('a_rep, 'a_pdb) val_param_type = Specs.sep * Specs.term
      let print_sep (sep,_) pads = Specs.print_sep sep pads
    end)
