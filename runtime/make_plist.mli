...
(*   type ('a_rep,'a_pdb, 'return) parser___ty       *)
(*   val parse : ('a_rep,'a_pdb) Pads.parser__ -> ('a_rep,'a_pdb, ('a_rep list, 'a_pdb Pads.pd list) Pads.parser__) parser_ty *)
end
...
module type Plist_print = sig
  ...
  (*   type ('a_rep,'a_pdb, 'return) print_sep_ty *)
  (*   val print_sep : ('a_rep, 'a_pdb, Pads.handle -> unit) print_sep_ty *)
end

module Make (Parse: Plist_parse) 
  (Print: Plist_print
   with type ('a_rep,'a_pdb) val_param_type = ('a_rep,'a_pdb) Parse.val_param_type) 
  : Type.TypeAndValParam 
      with type 'a_rep rep = 'a_rep list
      and  type 'a_pdb pd_body = 'a_pdb Pads.pd list
      and  type ('a_rep,'a_pdb) val_param_type = ('a_rep,'a_pdb) Parse.val_param_type
