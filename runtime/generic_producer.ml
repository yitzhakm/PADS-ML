...
module type BaseType = sig
  ...
  (** read from the source and produce a value in the basetype *)
...
module type S = sig
  ...
  (* Extensions get special treatment -- like records, the specializer
	 should give the producer the expected type tag and a default
	 value 
  *)
  module Extension : sig
	val process : source -> BTExtension.t -> string -> string * Pads.pd_header
  end
...
module Rec_ver = struct
  ...
  type     'source extension_t = 'source -> BTExtension.t -> string -> string * Pads.pd_header
  ...
  struct
	let producer = {
	  ...
	  process_int = Producer.Int.process;
	  process_float = Producer.Float.process;
	  process_char = Producer.Char.process;
	  process_string = Producer.String.process;
	  process_unit = Producer.Unit.process;

	  process_extension = Producer.Extension.process;

	  process_record = Producer.Record.process;
	  process_datatype = Producer.Datatype.process;
	  process_list = Producer.List.process;
	  process_table = Producer.Table.process;
	}
  end
end
