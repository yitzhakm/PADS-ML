(*
 *** MODIFIED ***
This file is a modified version of str.ml from the standard OCaml distribution.
It was modified by Yitzhak Mandelbaum on July 31st 2007.
*)
...
(*  the special exception on linking described in file strhack-LICENSE.*)
(*                                                                     *)
(***********************************************************************)

(* $Id: strhack.ml,v 1.3 2009/04/30 13:12:31 yitzhakm Exp $ *)
...
(*             failwith "\\( group not closed by \\)" *)
  ...
  and regexpclass2 c start i =
    ...
    if s.[i] = ']' && i > start then i+1 else begin
      ...
      end else if i+1 < len && c1 = '\\' && s.[i+1] = 'n' then begin
        Charset.add c '\n';
        regexpclass2 c start (i+2)
      end else if i+1 < len && c1 = '\\' && s.[i+1] = 't' then begin
        Charset.add c '\t';
        regexpclass2 c start (i+2)
      end else begin
        Charset.add c c1;
...
(*********************************************)
let pick_char cs = 
  let s = Charset.expand cs in    
  let rec search_string i =
    if i < 256 then
      if s.[i] = '\001' then Char.chr i else search_string (i+1)
    else
      raise Not_found
  in
    search_string 0

let rec make_canonical = function
    Char c -> String.make 1 c
  | String s -> s
  | CharClass cc -> String.make 1 (pick_char cc)
  | Seq rl -> String.concat "" (List.map make_canonical rl)
  | Alt (r1,_) -> make_canonical r1
  | Star r1 -> make_canonical r1
  | Plus r -> make_canonical r
  | Option r -> make_canonical r
  | Group (_,r) -> make_canonical r
  | Refgroup n -> raise Not_found
  | Bol -> ""
  | Eol -> ""
  | Wordboundary -> ""
