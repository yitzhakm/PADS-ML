...
module Datatype = struct
  ...
  let process_variant state variant_name variant_state =
	...
	"Section", indent::no_opt::Xml.Element ("Name", [], [Xml.PCData section_name])::xs -> 
	  state @ [Xml.Element (section_name, [], xs)]
      | "Some",_ | "None",_ | _ when (is_suffix variant_name "_anon") -> variant_state
